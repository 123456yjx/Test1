...
{
//	u8 res=0;
        if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)//接收到数据,读取寄存器非空 接收中断收到的数据必须是0x0d 0x0a结尾
    {
//        res=USART_ReceiveData(USART1);//（USART1->DR）;//读取接收到的数据
//        if(buf1_cnt<Buf1_Max&&((buf1_cnt&0x8000)!=0x8000))
//				{
//						Uart1_Buf[buf1_cnt]=res;//将接收到的数据存到缓存中
//					  buf1_cnt++;
//				}
...
void USART2_IRQHandler(void)                	
{
      ...
      if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET)//接收到数据
    {
        ...
        if(buf2_cnt<Buf2_Max&&((buf2_cnt&0x8000)!=0x8000))
				{
						Uart2_Buf[buf2_cnt]=res;//将接收到的数据存到缓存中
					  buf2_cnt++;
				}
